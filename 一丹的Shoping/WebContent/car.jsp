...
<body>
	...
	<!--navBox/-->
			...
			<c:forEach var="cars" items="${car}">
				<tr>
					<td class="common-text required"><c:out value="${cars.sName}" /></td>
						<td class="common-text required"><c:out value="${cars.sPrice}" /></td>
						 
				 <td class="common-text required">
				 <img src="upload/${cars.sPicture }" /></td>
				 <td class="common-text required"><c:out value="${cars.count}" /></td>
					...
					<c:choose>
						<c:when
							 test="${cars.count == 1}"> 
							<td><a href="deleteCar?id=${cars.id}">删除</a></td>
						</c:when>
						<c:when
							 test="${cars.count > 1}"> 
							<td><a href="deleteOneCar?id=${cars.id}">减少一件</a></td>
						</c:when>
			...
			<!--zongji/-->
			<div class="jiesuan">
				<a href="ShopList" class="jie_1">继续购物&gt;&gt;</a> <a
					href="administer_addorder" class="jie_2">立即结算&gt;&gt;</a>
				<div class="clears"></div>
			</div>
			<!--jiesuan/-->
			<div class="clears"></div>
